...
//   motorsMove();      //azimuth motor stepping
 ...
 if ((orbAzi > headingDegrees -1) && (orbAzi < headingDegrees +1)) {
    delay(8000);
   }
    else if  (orbAzi  > headingDegrees && orbAzi< 359) {          //elevation
      AZforwardStep();
    }
...
void AZforwardStep() {              //azimuth forward
  ...
  // Makes 1 pulse for making one step
  for (int y = 0; y < 1; y++) {       //make y < 100 for 100 steps at once, or y<1 for 1 step at a time
...
void AZbackwardStep() {               //azimuth backward
  ...
  // Makes 1 pulse for making one step
  for (int x = 0; x < 1; x++) {    //make x < 100 for 100 steps at once, or x<1 for 1 step at a time
    digitalWrite(stepPinX, HIGH);
    delayMicroseconds(2500);
    digitalWrite(stepPinX, LOW);
    delayMicroseconds(2500);
