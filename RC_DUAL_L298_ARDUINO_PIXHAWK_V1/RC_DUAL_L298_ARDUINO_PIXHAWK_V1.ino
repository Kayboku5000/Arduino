
// #### New L298 ####

#define spd3 3  // from pin 3 to pwm of hbridge
#define op5 14  // pin 1 of hbridge on pin 0
#define op6 15  // pin 2 of hbridge on pin 1

#define spd4 5  // from pin 5 to pwm of hbridge
#define op7 16  // pin 3 of hbridge on pin 2
#define op8 17  // pin 4 of hbridge on pin 4

// #### Original L298 ####
#define ip1 13   // input to pin5, from Pixhawk output 1
#define spd1 6  // from pin 6 to pwm of hbridge
#define op1 7  // pin 1 of hbridge on pin 7
#define op2 8  // pin 2 of hbridge on pin 8
int ch1;        // Value of channel 1 of the receiver
int val1;       // motor1 speed

#define ip2 9   // input to pin 9 from Pixhawk Output 3
#define spd2 10  // from pin 10 to pwm of hbridge
#define op3 11  // pin 3 of hbridge on pin 11
#define op4 12  // pin 4 of hbridge on pin 12
int ch2;        // Value of channel 2 of the receiver
int val2;       // motor2 speed


void setup() {
  pinMode (ip1, INPUT);  //pixhawk pwm
  pinMode (spd1, OUTPUT);
  pinMode (spd3, OUTPUT);
  pinMode (op1, OUTPUT);
  pinMode (op2, OUTPUT);
  pinMode (op5, OUTPUT);
  pinMode (op6, OUTPUT);

  digitalWrite(op1, LOW);
  digitalWrite(op2, LOW);
  digitalWrite(op5, LOW);
  digitalWrite(op6, LOW);

  pinMode (ip2, INPUT);  //pixhawk pwm
  pinMode (spd2, OUTPUT);
  pinMode (spd4, OUTPUT);
  pinMode (op3, OUTPUT);
  pinMode (op4, OUTPUT);
  pinMode (op7, OUTPUT);
  pinMode (op8, OUTPUT);
  digitalWrite(op3, LOW);
  digitalWrite(op4, LOW);
  digitalWrite(op7, LOW);
  digitalWrite(op8, LOW);
  Serial.begin(115200);
}

void loop() {

  ch1 = pulseIn(ip1, HIGH);    // Left Motor
  Serial.print(" ch1 ");
  Serial.print(ch1);

  ch2 = pulseIn(ip2, HIGH);    // Right Motor
  Serial.print(" ch2 ");
  Serial.println(ch2);


  //######### motor 1+2 #########



  if (ch1 < 1450 && ch1  > 950 )   // for reverse
  {
    val1 = map(ch1, 1450, 950, 25, 255);
    analogWrite(spd1, val1);
    analogWrite(spd3, val1);
    digitalWrite(op1, HIGH);
    digitalWrite(op2, LOW);
    digitalWrite(op5, HIGH);
    digitalWrite(op6, LOW);
  }

  else if (ch1 < 2200 && ch1 > 1550 )   // for forward
  {

    val1 = map(ch1, 1550, 2000, 25, 255);
    analogWrite(spd1, val1);
    analogWrite(spd3, val1);
    digitalWrite(op1, LOW);
    digitalWrite(op2, HIGH);
    digitalWrite(op5, LOW);
    digitalWrite(op6, HIGH);


  }

  else if (ch1 < 1550 && ch1 > 1450  ) //stop motor
  {
    digitalWrite(op1, LOW);
    digitalWrite(op2, LOW);
    digitalWrite(op5, LOW);
    digitalWrite(op6, LOW);



  }

  else if  (ch1 == 0)  //failsafe
  {
    digitalWrite(op1, LOW);
    digitalWrite(op2, LOW);
    digitalWrite(op5, LOW);
    digitalWrite(op6, LOW);

  }


  //######### motor 3+4 #########


  if  (ch2 < 1450 && ch2  > 950 )  // for reverse
  {
    val2 = map(ch2, 1450, 950, 25, 255);
    analogWrite(spd2, val2);
    analogWrite(spd4, val2);
    digitalWrite(op3, HIGH);
    digitalWrite(op4, LOW);
    digitalWrite(op7, HIGH);
    digitalWrite(op8, LOW);
  }

  else if  (ch2 < 2200 && ch2 > 1550 )  // for forward
  {

    val2 = map(ch2, 1550, 2000, 25, 255);
    analogWrite(spd2, val2);
    analogWrite(spd4, val2);
    digitalWrite(op3, LOW);
    digitalWrite(op4, HIGH);
    digitalWrite(op7, LOW);
    digitalWrite(op8, HIGH);

  }

  else if (ch2 > 1450 && ch2 < 1550)   //stop motor
  {
    digitalWrite(op3, LOW);
    digitalWrite(op4, LOW);
    digitalWrite(op7, LOW);
    digitalWrite(op8, LOW);

  }

  else if  (ch2 == 0)  //failsafe
  {
    digitalWrite(op3, LOW);
    digitalWrite(op4, LOW);
    digitalWrite(op7, LOW);
    digitalWrite(op8, LOW);


  }


  delay(10);

}
